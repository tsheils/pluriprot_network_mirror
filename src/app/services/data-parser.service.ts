...
export class DataParserService {
  ...
  private _fetchFile(url: string): any {
    return this._http.get<any[]>(url);
  }

  LoadData(): Observable<any> {
    return from(DATAFILES.map(file => {
      return this._fetchFile(file);
    })).pipe(
      map(res => {
        return res.pipe(
          map(response => {
  ...
  private _parseData(data: any) {
    const nodeObs = of(data.elements.nodes.map(node => {
      const n: Protein = this.nodeService.makeNode(node.data.name, {properties: node.data}) as Protein;
      if (!n.x && !n.y) {
        n.x = node.position.x;
        n.y = node.position.y;
      }
    ...
    const linkObs = of(data.elements.edges.map(edge => {
      const names = edge.data.name.split(' ');
      const source: Protein = this.nodeService.getById(names[0].trim()) as Protein;
      const target: Protein = this.nodeService.getById(names[2].trim()) as Protein;
      if (source.gene !== target.gene) {
